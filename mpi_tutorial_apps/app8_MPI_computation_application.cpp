...
void coordinator(int world_size) {
    //generqate random integers and store them in an array
    int num_ints = 20;
    int values[num_ints];
    for (unsigned int i = 0; i < num_ints; i++)
    {
        values[i] = rand() % 10;
        //std::cout << "rank 0, position " << i << " random value: " << values[i] << std::endl;
    }
    ...
    int partition_size = num_ints / world_size;
    ...
    //send a partition of data to each node
    
    for (unsigned int i = 1; i < world_size; i++)
    {   
        int *my_array = values + partition_size * i;
        
    //test

        // for (unsigned int i = 0; i < partition_size; i++)
        // {
        //     std::cout << "to send: " << my_array[i] << std::endl;
        // }
    //test     
        MPI_Send(my_array, partition_size,MPI_INT,i,0,MPI_COMM_WORLD);
        std::cout << "coordinator rank 0 send partition to rank: " << i << std::endl;
    ...
    average = (float) total / partition_size;
...
void participant(int world_rank, int world_size) {
    ...
    MPI_Bcast(&partition_size, 1, MPI_INT, 0, MPI_COMM_WORLD);
    std::cout << "participant rank " << world_rank << " recieved partition size of " << partition_size << std::endl;
    ...
    std::cout << "participant rank " << world_rank << " recieved partiton from root" << std::endl;
    
    // generate an average for our partition
    int total = 0;
    for(unsigned int i = 0; i < partition_size; i++){
        total += partition[i];
        //std::cout << "rank " << world_rank << " value: " << partition[i] << std::endl;
